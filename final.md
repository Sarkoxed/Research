...
ZKP - это доказательства, которые в одно время являются (чекнуть вики) убедительными и в то же время не раскрывают ничего, кроме факта верности доказываемого утверждения [1]. 
...
argument of knowledge(argument - comp, know - знание)
...
- Заявления о наличии знаниях(arguments of knowledge): Вы хотите доказать, что “вы знаете разложение N на простые множители”.
...
private coin protocol. honest verofier zero knowledge
...
написать что это ai, bi,, cio система ограничений
...
для чего библиотека и тд
перенести про две части работы циркома сюда

Он позволяет создавать массивные схемы, используя много маленьких компонентов.
...
## 2.3 Пример проведения церемонии, доказательства и проверки.
...
вообще церемонию не проводят а используют доверенно проведенную. Сейчас тест.
...
Если проблема была в системе доказательств, в зависимости от значимости ошибки, это может вызвать множество неудобств, как для производителя, так и для пользователя. В основном производителю придется вносить радикальные изменения в ПО и удостовериться, что пользователи переустановят приложение.

Очень важно выявлять такие ошибки еще во время написания по. Однако, так как отрасль довольно новая и представлено немного классов уязвимостей таких приложений, (переделать плохой вывод)требуются различные подходы к тестированию такого ПО. Одним из них является верификация частей схемы с использованием символического исполнения.
...
Символическое исполнение - это метод тестирования(не только лол) программного обеспечения, который полезен для генерации тестовых данных и подтверждения качества программы. Исполнение требует выбора путей, которые осуществляются набором значений данных. Программа, которая выполняется с использованием фактических данных, приводит к выводу ряда значений. В символическом исполнении данные заменяются символическими значениями с набором выражений, по одному выражению на выходную переменную. [11]
конкалическое исполнение
проверить свойство программы
то что две программы всегда приводят к одинаковым значениям
...
Большим преимуществом верификации является то, что она невилирует определенный класс ошибок, которые может быть проблематично искать человеку вручную. (переформулировать)
